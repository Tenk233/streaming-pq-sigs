...
{
  #if defined(SAM3X8E)
    while ((UART_SR & UART_SR_RXRDY) != UART_SR_RXRDY);
    return (char) UART_RHR;
  #elif defined(STM32F207ZG)
    return (char) usart_recv_blocking(SERIAL_USART);
  #else
    #error hal_get_char not implemented for this platform
  #endif
...
void hal_send_int_raw(const u32 n) {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      #else
        #error hal_send_int_raw not implemented for this platform
      #endif
    }
}
...
void hal_send_bytes(const char* in, u32 len)
{
  size_t i;
#if defined(SAM3X8E)
  for (i = 0; i < len; ++i)
...
{
  /* sys_tick_handler gets called every 0xFFFFFF ticks,
   * see `systick_set_reload(0xFFFFFFu);` call in systick_setup.
   * Since the systick value counts downwards, we want to have
   * (overflowcnt+1) * (0xFFFFFF + 1) - systick_value.
   */
  while (true) {
...
void stream_send_request(u32 offset, u32 size, stream_chunk_type type) {
  hal_send_byte(STREAM_CHUNK_REQUEST);
  hal_send_byte(type);
  hal_send_int_raw(offset);
  hal_send_int_raw(size);
}

void stream_init(){
  /* Wait in busy loop until host is up */
  hal_get_char();
  hal_send_byte(STREAM_INIT_MSG);
}
...
void stream_send_str(const char *pkg) {
  ...
  hal_send_byte(STREAM_DEBUG_STR);
  hal_send_int_raw(len);
  hal_send_str(pkg);
}
...
void stream_send_bytes(const char *name,const char *pkg, u32 len) {
  ...
  hal_send_byte(STREAM_DEBUG_BYTES);
  hal_send_int_raw(str_len);
  hal_send_str(name);
  hal_send_int_raw(len);
  hal_send_bytes(pkg, len);
}

void stream_send_benchmark(const char *name, u64 result) {
  hal_send_byte(STREAM_BENCHMARK);
  hal_send_int_raw(strlen(name));
  hal_send_str(name);
  hal_send_long_long_raw(result);
}

void stream_send_result(u32 result) {
  hal_send_byte(STREAM_RESULT);
  hal_send_int_raw(result);
}
