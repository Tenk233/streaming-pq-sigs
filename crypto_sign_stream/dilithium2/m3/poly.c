...
void poly_challenge(poly *c, const uint8_t seed[SEEDBYTES]) {
  ...
  shake256_inc_squeeze(buf, 8, &state);
...
void polyw1_pack_absorb(shake256incctx *state, const poly *a){
    ...
    for(i = 0; i < N/4; ++i){
      buf[i*3+0]  = a->coeffs[4*i+0];
      buf[i*3+0] |= a->coeffs[4*i+1] << 6;
      buf[i*3+1]  = a->coeffs[4*i+1] >> 2;
      buf[i*3+1] |= a->coeffs[4*i+2] << 4;
      buf[i*3+2]  = a->coeffs[4*i+2] >> 4;
      buf[i*3+2] |= a->coeffs[4*i+3] << 2;
    }
